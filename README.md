...
isolation register stage after every K stages) in Verilog from the C++ code.
...
unzip CS19S018.zip ; cd CS19S018
...
**Block Diagram and Output for N=8 and K=2**

**Figure(1)**

• The above figure shows the resulting block diagram of pipelined Wallace Tree

Multiplier generated by HLL code-named Multiplier\_gen\_pipeline.cpp with N=8 and

K=2.

• Note that code does not generate 16-bit CSA at every stage in the CSA tree. Instead,

it generates less bit size CSA wherever possible. **This is done in order to optimize**

**the area of the circuit.**

• P1, P2 .. P8 are the partial products that are generated from multiplicand and

multiplier.

• Register stage generated by the code is shown in the figure above.





• 3-bit left Shifter are used to shift the CSA inputs appropriately. This is needed when

CSA has input values with different shifts in it.

• Way the CSAs are connected, and the structure of the CSA tree, which is generated

by the HLL code is clearly shown in the above figure.
