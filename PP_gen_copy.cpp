...
int main(){
  ...
  if( filePointer == NULL ) 
	...
	if( (numofremain + numofremainprev) % 3 == 0){
		...
		if(numofremainprev != 0){
			
			//cout << numofremain << " "<< numofCSAs << " " << numofremainprev <<  " "<< prevflag <<"\n";
			...
			numofremainprev = 0;
			// need to recalculate numofCSAs and numofremain;
			numofremain = numofremain + ((numofCSAs*3 - flag) % 3);
			numofCSAs = (numofCSAs*3 - flag) / 3;
			//prevflag = flag;
	...
	for(int i=0;i<numofCSAs;i++){
		...
		if(flag == 1){
		val -= 3;
		fprintf(filePointer,"wire [%d:0]l%d%d,l%d%d;\n",2*n-1,stage+1,2*(i+1)+1,stage+1,2*(i+1)+2);
		fprintf(filePointer,"CSA c%d(l%d%d,l%d%d,l%d%d,l%d%d,l%d%d);\n",CSAnum,stage,3*i+2,stage,3*i+3,stage,3*i+4,stage+1,2*(i+1)+1,stage+1,2*(i+1)+2);
		CSAnum++;
		val += 2;
		}
		if(flag == 2){
		val -= 3;
		fprintf(filePointer,"wire [%d:0]l%d%d,l%d%d;\n",2*n-1,stage+1,2*(i+1)+1,stage+1,2*(i+1)+2);
		fprintf(filePointer,"CSA c%d(l%d%d,l%d%d,l%d%d,l%d%d,l%d%d);\n",CSAnum,stage,3*i+3,stage,3*i+4,stage,3*i+5,stage+1,2*(i+1)+1,stage+1,2*(i+1)+2);
		CSAnum++;
		val += 2;
		}
	}
