...
// module to generate full adder ends here...
	...
	int bitcsa;		// store the bit length of the CSA that needs to be used at some particular point in the CSA tree.
	int maxshift;	
	int minshift;
	int temp;
	int CSAnum=1;
	...
	// This while loop will execute and will halt only when CSA tree reduces the N numbers (N partial products) to the two numbers. which are then added using the CLA.  
	while(q1.size() != 2){
		i1 = q1.front();
		q1.pop();
		i2 = q1.front();
